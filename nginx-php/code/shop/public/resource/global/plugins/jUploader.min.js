...
(function($) {
    window.jUploader = $.jUploader = function(options) {
        ...
        var createInput = function() {
            ...
            if (options.type == 'image') {
                ...
                accept = 'video/mp4,video/avi,video/mpeg,video/rmvb,video/mov,video/3gp,video/rm,video/asf,video/wmv,video/flv,video/swf'
            } else if (options.type == 'audio') {
                accept = 'audio/mp3,audio/wma,audio/wav,audio/asf,audio/acc,audio/ogg'
            }
            var input = $('<input type="file" onchange="this.blur();" accept="' + accept + '" ' + (options.multiple ? 'multiple': '') + '/>');
            input.attr("id", 'jUploader-file' + options.id).attr("name", (options.multiple ? 'jUploaderFile[]': 'jUploaderFile')).css({
                position: 'absolute',
                right: 0,
                top: 0,
                margin: 0,
            ...
            function(e) {
                if ($('#jUploader-file1')[0].files.length > 15) {
                    alert('最多上传15张');
                    return false
                };
                var check = true;
                $.each($('#jUploader-file1')[0].files,
                function(n, value) {
    ...
    $.jUploader.setDefaults = function(defaults) {
        $.jUploader.defaults = $.extend({},
        $.jUploader.defaults, defaults)
    };
